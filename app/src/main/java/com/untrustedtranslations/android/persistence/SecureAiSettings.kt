...
data class AiSettings(
    ...
    /**
     * Whether the shared sound-effect heuristic (ComicDialogueDetector.looksLikeSoundEffect) is
     * allowed to drop anything at all. Defaults to false -- matching manga-image-translator's
     * own "ignore_bubble" setting, which ships defaulted to 0 (off) in the actual reference
     * tool this project is modeled on, with filtering something users opt into once they've
     * actually seen SFX bleeding into their translations, not something applied by default.
     * Erring toward keeping text is the safer failure mode: losing real dialogue/narration by
     * default (which several rounds of testing in this exact app kept hitting) is worse than
     * occasionally translating a "BAM!".
     */
    val sfxFilterEnabled: Boolean = false,
)
...
object SecureAiSettings {
    ...
    private const val KEY_SFX_FILTER = "sfx_filter_enabled"
    ...
    fun load(context: Context): AiSettings {
        ...
        return AiSettings(
            ...
            sfxFilterEnabled = prefs.getBoolean(KEY_SFX_FILTER, false),
        )
    }
    ...
    fun save(context: Context, settings: AiSettings) {
        val editor = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
            ...
            .putBoolean(KEY_SFX_FILTER, settings.sfxFilterEnabled)
