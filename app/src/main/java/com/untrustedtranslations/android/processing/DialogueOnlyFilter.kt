    ...
    suspend fun keepDialogue(
        ...
        try {
            ...
            blocks.filter { block ->
                ...
                val matchedRegion = regions.firstOrNull { region ->
                    region.rect.contains(rect.centerX(), rect.centerY()) ||
                        overlapFraction(rect, region.rect) >= .30f
                } ?: return@filter false
                !(matchedRegion.isFreeText &&
                    ComicDialogueDetector.looksLikeSoundEffect(
                        block.originalText, rect, bitmap.width, bitmap.height,
                        bitmap = bitmap, allRegions = regions,
                    ))
            }
        } finally {
            bitmap.recycle()
