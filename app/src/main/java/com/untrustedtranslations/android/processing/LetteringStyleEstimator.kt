...
object LetteringStyleEstimator {
    fun estimate(
        ...
        // Use the detected border color as background so translated text covers JP text cleanly
        val bgArgb = (0xFF000000L or (android.graphics.Color.red(background).toLong() shl 16)
                or (android.graphics.Color.green(background).toLong() shl 8)
                or android.graphics.Color.blue(background).toLong())
        return TextStyle(
            ...
            bold = inkRatio > .11f,
            vertical = vertical,
            textColorArgb = if (luminance(background) > 128f) 0xFF000000L else 0xFFFFFFFFL,
            strokeColorArgb = if (luminance(background) > 128f) 0xFFFFFFFFL else 0xFF000000L,
            strokeWidthSp = estimatedSp * 0.15f,
