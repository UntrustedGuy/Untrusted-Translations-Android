...
internal object MangaOcrPageEngine {
    ...
    // The upstream manga-ocr model's own generation_config.json specifies max_length: 300 --
    // this was hard-capped at 64, roughly a fifth of what the model was actually trained/tuned
    // to generate, silently truncating any longer dialogue or narration block mid-sentence.
    // The loop below already exits as soon as every item in the batch hits its end token, so
    // raising this only costs extra compute on genuinely long text, not on the common short case.
    ...
    suspend fun process(
        ...
        try {
            ...
            val regions = ComicDialogueDetector.detect(
                ...
                bitmap,
                minimumScore = if (deepScan) .15f else .25f,
                pageKey = page.originalSource.toString(),
            )
            val detectorMs = elapsedMs(detectorStarted)

            val environment = OnnxSessionCache.environment
            val mergedRegions = regions.toMutableList()

            // Supplement with DBNet if RapidOCR is installed (to catch free text)
            val rapidOcrDir = ModelPackManager.directory(context, ModelPackId.RAPID_OCR_JAPANESE)
            val detectorFile = java.io.File(rapidOcrDir, "det.onnx")
            if (detectorFile.isFile) {
                try {
                    val detector = OnnxSessionCache.getOrCreate("rapid_ocr_v4_det", detectorFile)
                    val dbnetRegions = RapidOcrPageEngine.detect(environment, detector, bitmap)
                    
                    // Group nearby DBNet lines into larger blocks to prevent fragmented text
                    val groupedDbnet = mutableListOf<android.graphics.Rect>()
                    for (dbRegion in dbnetRegions) {
                        val rect = dbRegion.rect
                        var merged = false
                        for (i in groupedDbnet.indices) {
                            val group = groupedDbnet[i]
                            val dx = maxOf(0, maxOf(group.left, rect.left) - minOf(group.right, rect.right))
                            val dy = maxOf(0, maxOf(group.top, rect.top) - minOf(group.bottom, rect.bottom))
                            val distance = kotlin.math.sqrt((dx * dx + dy * dy).toDouble())
                            if (distance < bitmap.width * 0.04) { // Merge if within 4% of page width
                                group.union(rect)
                                merged = true
                                break
                            }
                        }
                        if (!merged) groupedDbnet.add(android.graphics.Rect(rect))
                    }

                    val pageArea = bitmap.width.toLong() * bitmap.height
                    for (dbRect in groupedDbnet) {
                        val regionArea = dbRect.width().toLong() * dbRect.height()
                        if (regionArea > pageArea * 0.12f) continue
                        if (regionArea < pageArea * 0.001f) continue
                        val overlapsExisting = mergedRegions.any { existing ->
                            overlapOverSmaller(dbRect, existing.rect) > 0.35f
                        }
                        if (!overlapsExisting) {
                            mergedRegions += ComicDialogueDetector.Region(dbRect, 0.8f)
                        }
                    }
                } catch (e: Exception) {
                    Log.w("MangaOCR", "DBNet supplemental failed", e)
                }
            }
            ...
            val crops = mergedRegions.map { region ->
            ...
            val blocks = crops.indices.mapNotNull { index ->
                ...
                if (reading.text.isBlank() || reading.confidence < .18f || (reading.text.length == 1 && !reading.text.first().isLetter())) return@mapNotNull null
                val region = crops[index].region
                if (region.isFreeText &&
                    ComicDialogueDetector.looksLikeSoundEffect(
                        reading.text, region.rect, bitmap.width, bitmap.height,
                        bitmap = bitmap, allRegions = mergedRegions,
                    )
    ...
    private fun overlapOverSmaller(a: android.graphics.Rect, b: android.graphics.Rect): Float {
        val intersectionWidth = (kotlin.math.max(0, kotlin.math.min(a.right, b.right) - kotlin.math.max(a.left, b.left)))
        val intersectionHeight = (kotlin.math.max(0, kotlin.math.min(a.bottom, b.bottom) - kotlin.math.max(a.top, b.top)))
