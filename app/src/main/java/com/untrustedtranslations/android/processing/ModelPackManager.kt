...
object ModelPackManager {
    ...
    val packs = listOf(
        ModelPackInfo(
            ModelPackId.LAMA_INPAINTING,
            "LaMa Inpainting",
            "Perfect generative inpainting and text removal.",
            305, 3, "Apache-2.0 LaMa and CTD models."
        ),
        ModelPackInfo(
            ModelPackId.COMIC_DIALOGUE_DETECTOR,
            "Comic dialogue detector",
            "Shared speech-bubble text detector used by every OCR recognizer. Keeps dialogue and free text (captions/thoughts), drops empty bubble outlines. Sound-effect filtering is optional -- off by default, toggle it in AI Settings.",
    ...
    class HashMismatchException(val packId: ModelPackId, val fileName: String, val expected: String, val actual: String) : Exception("Security check failed for $fileName.")

    suspend fun download(
        context: Context,
        id: ModelPackId,
        bypassHashCheck: Boolean = false,
            ...
            definitions.forEachIndexed { index, definition ->
                ...
                definition.sha256?.let { expected ->
                    ...
                    if (!actual.equals(expected, ignoreCase = true)) {
                        if (!bypassHashCheck) {
                            partial.delete()
                            throw HashMismatchException(id, definition.name, expected, actual)
                        } else {
                            Log.w("ModelPackManager", "Bypassing hash check for ${definition.name}.")
                        }
                    }
                }
    ...
    private fun files(id: ModelPackId): List<PackFile> = when (id) {
        ModelPackId.LAMA_INPAINTING -> listOf(
            PackFile(
