...
object RemoteTranslationEngines {
    suspend fun unofficialGoogle(text: String, source: String, target: String): String =
        withContext(Dispatchers.IO) {
            ...
            val endpoint =
                "https://translate.googleapis.com/translate_a/t?client=dict-chrome-ex&sl=$source&tl=$target&q=$query"
            val response = request(endpoint, "GET")
            if (response.trim().startsWith("<")) error("Google temporarily blocked your IP from the unofficial API.")
            val root = JSONArray(response)
            root.optString(0).orEmpty().ifBlank { error("Unofficial Google Translate returned an empty translation.") }
        }
